...
- Fresh verification: `uv run pytest tests/integration/test_migrations.py tests/inventory -q` and `uv run pyright src/placegame/inventory src/placegame/game tests/inventory` must succeed before handing the module to WebUI work. The later deployment plan and GitHub Actions own Compose validation.
